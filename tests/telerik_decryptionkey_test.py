...
def test_PBKDF2_crypt():
    ...
    for testing_encryption_key in testing_encryption_keys:
        ...
        known_good_secret_PBKDF2 = urllib.parse.unquote(known_good_dialogparameters_PBKDF2)[:-44]

        r = x.telerik_decrypt(derivedKey, derivedIV, base64.b64decode(known_good_secret_PBKDF2))
        assert r
        assert (
            r
...
def test_PBKDF1_MS_crypt():
    ...
    for testing_encryption_key in testing_encryption_keys:
        ...
        known_good_secret_PBKDF1_MS = urllib.parse.unquote(known_good_dialogparameters_PBKDF1_MS)[:-44]

        r = x.telerik_decrypt(derivedKey, derivedIV, base64.b64decode(known_good_secret_PBKDF1_MS))
        assert r
        assert (
            r
